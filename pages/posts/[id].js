...
import postsPageInitialPathsD6d59Resource from '../../resources/posts-page-initial-paths-d6d59'
import postsPageInitialProps1d0e6Resource from '../../resources/posts-page-initial-props-1d0e6'
  ...
  return (
    <>
      <div className="posts11-container">
        <Head>
          <title>Posts1 - Customer Intranet Architect</title>
          <meta
            property="og:title"
            content="Posts1 - Customer Intranet Architect"
          />
        </Head>
        <DataProvider
...
export async function getStaticPaths() {
  try {
    const response = await postsPageInitialPathsD6d59Resource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await postsPageInitialProps1d0e6Resource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
